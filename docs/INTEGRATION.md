...
## 5) Offline quality evaluation (не влияет на продуктовый флоу)

Validation dataset (golden set) используется только для оффлайн‑оценки качества внешней LLM и регрессионных проверок.
Он не участвует в обработке пользовательских запросов и не требует изменений в точках интеграции Mini App backend.
