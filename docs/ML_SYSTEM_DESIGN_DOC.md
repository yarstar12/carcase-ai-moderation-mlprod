...
- Что считаем успешным пилотом и как развиваемся дальше
  ...
  - Дальше: расширяем покрытие, улучшаем policy/prompt, автоматизируем формирование golden set, оптимизируем стоимость.
...
| Golden set — ручная разметка сэмплов | Будет создано | Ops + ML Engineering | нет (процесс разметки) |
...
- план формирования golden set: размер, стратификация, ответственные

Риски и что делаем:
- при отсутствии размеченной “истины” опираемся на контур `review` и формируем golden set как минимальный источник правды
...
Оценка:
- собираем выборку реальных попыток создания/редактирования
- размечаем golden set и считаем baseline метрики для `block` — precision/recall
...
Оценка:
- на golden set считаем precision/recall для `block` и долю `review`
...
Качество:
- `precision_block` ≥ 0.90 на golden set. Это стартовая цель; уточняется после baseline.
