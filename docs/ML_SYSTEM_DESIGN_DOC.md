...
- staged rollout (canary‑подход): включаем влияние модерации на продукт постепенно, со стабильным сплитом по user_id:
  1) 0% (выключено)
  2) 10% — только создание клана (минимум 3 календарных дня)
  3) 10% — создание + редактирование описания (минимум 3 календарных дня)
  4) 50% — создание + редактирование (минимум 7 календарных дней)
  5) 100% — создание + редактирование (период стабилизации 14 календарных дней)

Гейты (перед увеличением процента трафика):
- smoke‑прогон validation dataset (golden set) как регрессионный контроль и сравнение с предыдущей стабильной версией `policy/prompt/model`:
  - smoke‑прогон — ежедневно 1 раз в сутки;
  - full‑прогон — 1 раз в неделю и дополнительно перед переходом на следующий шаг rollout;
  - `precision_block` ≥ 0.90 и не хуже baseline − 2 п.п.;
  - `critical_fn_rate` (expected=block → predicted=allow) ≤ 1% и не хуже baseline + 0.5 п.п.;
  - `review_rate` не выше baseline + 5 п.п.
- production‑аудит качества на реальном трафике (см. §3.1–3.2):
  - размечено минимум 200 кейсов `allow` и 50 кейсов `block` на каждом шаге rollout;
  - 0 критичных пропусков (в пилоте лучше остановиться и расширить `review`, чем пропустить нарушение в публичные рейтинги);
  - уровень ложных блокировок не приводит к падению конверсии относительно контроля.
...
- качество на validation dataset (оффлайн):
  - smoke‑набор: ежедневно 1 раз в сутки;
  - full‑набор: 1 раз в неделю и перед каждым увеличением процента rollout;
...
- human truth — решения модератора по `review` + выборочный аудит авто‑решений:
  - чтобы оценивать качество на реальном распределении, выборочно проверяем `allow` и `block` вручную по фиксированному регламенту;
  - это нужно, потому что только `review` даёт смещённую выборку (пограничные кейсы).

Определения метрик пилота (как считаем и с чем сравниваем):
- `precision_block` (оффлайн, validation dataset) — доля корректных блокировок среди всех предсказанных `block`: `TP_block / (TP_block + FP_block)`.
- `critical_fn_rate` (оффлайн, validation dataset) — доля критичных пропусков: среди примеров с `expected=block` какая доля получила `predicted=allow`.
- `prod_critical_fn_rate` (прод‑аудит, реальный трафик) — среди случайной выборки из `predicted=allow` какая доля по решению человека должна быть `block`. На практике в пилоте используем ещё более строгий критерий: **количество** критичных пропусков (`prod_critical_fn_count`) на размеченной выборке должно быть 0.
- `prod_false_block_rate` (прод‑аудит) — среди выборки из `predicted=block` какая доля по решению человека должна быть `allow`.
- `override_rate` (прод‑аудит/`review`) — доля кейсов, где решение человека ≠ решению модели. Используется как показатель “насколько часто модель ошибается/сомневается” и как сигнал для донастройки policy/prompt.
- Сравнение делаем в одинаковых временных окнах и в одном и том же сплите пользователей: тест vs контроль (feature flag), а также текущая версия `policy/prompt/model` vs предыдущая стабильная на одном и том же наборе данных (validation dataset и production‑audit выборка).

Минимальный объём разметки для перехода на следующий шаг rollout (go/no-go):
- `allow` аудит: не менее 200 кейсов на шаг rollout;
- `block` аудит: не менее 50 кейсов на шаг rollout;
- `review`: 100% кейсов, так как без решения модератора они не должны попадать в публикацию/применение.
- для шагов 10% допускается накопление выборки до 7 дней; для шагов 50% и 100% выборка собирается в пределах 3 дней.

Пост‑релизная проверка качества на реальном трафике:
- после каждого шага staged rollout и после выхода на 100% собираем production‑audit набор размеченных кейсов (реальные UGC → решение модели → решение человека);
- разметка формируется через стратифицированный аудит авто‑решений:
  - на шагах 10% и 50% — ежедневно размечаем 70 кейсов `allow` и 20 кейсов `block`;
  - на шаге 100% (период стабилизации) — ежедневно размечаем 100 кейсов `allow` и 30 кейсов `block`;
  - после стабилизации на 100% — еженедельно размечаем 300 кейсов `allow` и 100 кейсов `block`;
  - приоритетно добавляем “верхний риск” (подозрительные паттерны по правилам/длине/языку), чтобы быстрее ловить критичные пропуски.
- аудит `allow/block` допускается проводить постфактум (после публикации), не влияя на UX; при нахождении нарушения применяется процедура пост‑модерации (скрытие/переименование/санкции).
- по размеченному прод‑набору считаем метрики и сравниваем:
  - тест vs контроль (feature flag) в одинаковых временных окнах;
  - текущая версия `policy/prompt/model` vs предыдущая стабильная (по времени и по шагам rollout).
...
- качество на реальном трафике (по production‑аудиту):
  - `prod_critical_fn_rate` (случаи, когда модель дала `allow`, а человек считает, что нужно `block`) — стремится к 0. Критерий гейта пилота: **0 критичных пропусков** на размеченной выборке шага rollout (если найден хотя бы 1 — не увеличиваем трафик, при необходимости откатываемся на предыдущий шаг и/или расширяем `review`);
  - `prod_false_block_rate` (лишние блокировки, когда модель дала `block`, а человек считает, что нужно `allow`) — ≤ 10% на размеченной выборке; дополнительно контролируем конверсию и жалобы пользователей (если конверсия падает > 1–3 п.п. относительно контроля, считаем это провалом гейта).
...
- Production audit evaluation: batch‑джоба считает метрики качества по размеченным человеком кейсам с реального трафика (стратифицированный аудит авто‑решений `allow/block`), чтобы контролировать качество после релиза на настоящих пользовательских текстах.
...
- эндпоинт `POST /moderate`:
  - SLO доступности сервиса: не ниже 99.5% в месяц;
  - целевая пропускная способность: до 5 RPS на пике (с запасом относительно текущего прогноза нагрузки);
...
- SLO batch‑контуров:
  - отчёт `daily report` должен быть сформирован и доступен в S3 до 08:00 UTC следующего дня;
  - smoke‑evaluation должен завершаться в пределах 60 минут после старта DAG.
...
Расчётная модель затрат (базовый сценарий, в месяц):
- предпосылки:
  - online: 100–350 LLM‑запросов/день;
  - smoke‑evaluation: 300 запросов/день;
  - full‑evaluation: 10 000 запросов 1 раз в неделю;
  - тариф `gpt-5.4-nano` (Standard): input $0.20 / 1M tokens, output $1.25 / 1M tokens;
  - в базовый бюджет не закладываем cached input как обязательную оптимизацию; если prompt caching будет доступен в проде, фактическая стоимость окажется ниже;
  - оценка токенов на 1 проверку:
    - `squad_name`: ~140 input tokens + ~20 output tokens;
    - `squad_description`: ~160 input tokens + ~20 output tokens;
    - для бюджетирования используем округлённую оценку с запасом: ~200 input tokens + ~25 output tokens на один LLM‑запрос;
  - стоимость 1 LLM‑запроса при такой оценке: `200 * $0.20 / 1M + 25 * $1.25 / 1M ≈ $0.00007`; в финансовом плане округляем до ~$0.00010 на запрос;
  - стоимость одной продуктовой операции:
    - создание клана: 2 запроса (название + описание) → ~$0.00014 базово, ~$0.00020 с запасом;
    - редактирование описания: 1 запрос → ~$0.00007 базово, ~$0.00010 с запасом.
- LLM‑затраты:
  - online: ~3 000–10 500 запросов/мес → ~$0.21–$0.74 базово, ~$0.30–$1.05 в бюджете с запасом;
  - smoke: ~9 000 запросов/мес → ~$0.63 базово, ~$0.90 в бюджете;
  - full: ~40 000 запросов/мес → ~$2.80 базово, ~$4.00 в бюджете;
  - итого LLM: ориентир ~$3.6–$4.2/мес по базовой оценке или ~$5.2–$6.0/мес в бюджетной модели с запасом.
- инфраструктура:
  - сервис + мониторинг + Airflow (1 небольшая VM на этапе MVP, при необходимости масштабирования — 2 VM): ~$15–$35/мес;
  - Postgres: ~$10–$20/мес;
  - S3/MinIO и трафик отчётов: ~$1–$5/мес.
- ручная модерация `review`:
  - при `review_rate` ≤ 10% и текущем трафике: единицы или низкие десятки кейсов в день, ориентир ~2–8 часов/мес.
  - при ставке $5/час: ~$10–$40/мес.
- итоговый ориентир полной стоимости: ~$41–$106/мес (LLM + инфраструктура + review‑операции).
- контроль бюджета:
  - hard cap на LLM‑запросы/день и алерты на достижение 70–80% лимита;
  - автоматическое ограничение evaluation через `--max-examples`.
...
Отказоустойчивость и сценарии деградации:
- если недоступен `moderation-service`, product backend не публикует текст “вслепую”: rollout можно быстро уменьшить или выключить через feature flags, а спорные кейсы переводятся в безопасный режим `review`;
- если недоступен OpenAI API или внешний классификатор возвращает ошибку/невалидный ответ, online-контур деградирует в `review`; rule-based проверки продолжают работать;
- если ухудшилось качество модели, rollout не расширяется, текущая версия `policy/prompt/model` откатывается на предыдущую стабильную, а доля `review` временно увеличивается;
- если растёт backlog очереди `review`, rollout останавливается или уменьшается, а policy/rules/prompt донастраиваются до возврата к целевому SLA;
- если недоступен Postgres для аудита, пользовательский флоу модерации не должен полностью останавливаться: инцидент поднимается через алерты, а хранилище восстанавливается отдельно;
- если недоступны S3/MinIO, Airflow или batch-задачи, online-модерация продолжает работать; batch-пайплайн после восстановления перезапускается безопасно за счёт идемпотентности и backfill;
- на уровне single-node MVP базовая устойчивость обеспечивается stateless-дизайном сервиса, healthcheck и restart policy контейнера; для production-окружения целевая схема — несколько реплик сервиса и оркестрация уровня Kubernetes.
