...
- предпосылки:
  ...
  - усреднённая стоимость 1 LLM‑запроса для модерации (вход+выход): ~$0.001 (консервативная оценка для бюджетирования).
- LLM‑затраты:
  - online: ~3 000–10 500 запросов/мес → ~$3–$11;
  - smoke: ~9 000 запросов/мес → ~$9;
  - full: ~40 000 запросов/мес → ~$40;
  - итого LLM: ориентир ~$52–$60/мес.
- инфраструктура:
  - сервис + мониторинг + Airflow (1–2 VM): ~$30–$60/мес;
  - managed Postgres: ~$15–$30/мес;
  - S3/MinIO и трафик отчётов: ~$5–$15/мес.
- ручная модерация `review`:
  - при `review_rate` ≤ 10% и текущем трафике: десятки кейсов/день, ориентир ~5–20 часов/мес.
  - при ставке $5/час: ~$25–$100/мес.
- итоговый ориентир полной стоимости: ~$127–$265/мес (LLM + инфраструктура + review‑операции).
- контроль бюджета:
