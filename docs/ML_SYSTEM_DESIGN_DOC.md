...
- разметка формируется через стратифицированный аудит авто‑решений:
  - регулярно выбираем случайную выборку из `allow` и `block` (на пилоте — ежедневно; после стабилизации — реже);
...
- качество на реальном трафике (по production‑аудиту):
  - `prod_critical_fn_rate` (случаи, когда модель дала `allow`, а человек считает, что нужно `block`) — близко к 0; в пилоте используем жёсткий порог и при превышении откатываемся на предыдущий шаг rollout;
  - `prod_false_block_rate` (лишние блокировки, когда модель дала `block`, а человек считает, что нужно `allow`) — удерживаем на уровне, не ухудшающем конверсию относительно контроля.
